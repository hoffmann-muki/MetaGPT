...
class Optimizer:
        ...
        with span(
            ...
            if mode == "Test":
                ...
                for i in range(test_n):
                    ...
                    with span(label("aflow.test_iteration", index=i + 1), color="blue"):
                        score = loop.run_until_complete(self.test())
                return None

            for opt_round in range(self.max_rounds):
                with span(
                    label("aflow.round", index=opt_round + 1, current_round=self.round),
                    color="blue",
                ):
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    ...
                    while retry_count < max_retries:
                        try:
                            ...
                            if retry_count == max_retries:
                                logger.info(
                                    "Max retries reached. Moving to next round."
                                )
                                score = None
    ...
    async def _optimize_graph(self):
        with span(
            label("aflow.optimize_graph", dataset=self.dataset, round=self.round),
            color="blue",
        ):
            validation_n = (
                self.validation_rounds
            )  # validation datasets's execution number
            graph_path = f"{self.root_path}/workflows"
            data = self.data_utils.load_results(graph_path)

            if self.round == 1:
                directory = self.graph_utils.create_round_directory(
                    graph_path, self.round
                )
            ...
            while True:
                ...
                graph_optimize_node = await ActionNode.from_pydantic(
                    GraphOptimize
                ).fill(
                    context=graph_optimize_prompt,
                    mode="xml_fill",
                    llm=self.optimize_llm,
                )

                response = await self.graph_utils.get_graph_optimize_response(
                    graph_optimize_node
                )
