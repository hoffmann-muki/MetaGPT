...
class EvaluationUtils:
        ...
        with span(
            ...
            for i in range(validation_n):
                with span(
                    ...
                    cur_round = (
                        optimizer.round + 1 if initial is False else optimizer.round
                    )
    ...
    async def evaluate_graph_test(self, optimizer, directory, is_test=True):
        with span(
            label(
                "aflow.evaluate_graph_test", dataset=optimizer.dataset, is_test=is_test
            ),
            color="blue",
        ):
            evaluator = Evaluator(eval_path=directory)
